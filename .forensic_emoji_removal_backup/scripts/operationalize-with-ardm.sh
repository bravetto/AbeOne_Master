...
if [ ! -f "$ARDM_META_INTEGRATION" ]; then
    echo -e "${YELLOW}⚠️  ARDM integration not found, running standard operationalization...${NC}"
...
if [ -n "$CONVERSATION_CONTEXT" ]; then
    ...
    if [ -n "$ARDM_RESULT" ]; then
        ...
        if [ "$TOTAL_ITEMS" -gt 0 ]; then
            echo -e "${YELLOW}⚠️  ARDM detected $TOTAL_ITEMS actionable item(s)${NC}"
            echo ""
    ...
    echo -e "${YELLOW}⚠️  No conversation context available for ARDM scan${NC}"
fi
