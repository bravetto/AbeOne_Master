...
if [ -f "scripts/operationalize.sh" ]; then
    ...
    echo -e "${YELLOW}  Operationalize script not found, skipping...${NC}"
fi
echo ""
...
if command -v python3 &> /dev/null; then
    ...
    for module in "${REQUIRED_MODULES[@]}"; do
        if python3 -c "import $module" 2>/dev/null; then
            ...
            echo -e "${YELLOW}  Python module missing: $module${NC}"
        fi
    done
else
...
if [ -x ".claude/hooks/session-start.sh" ]; then
    ./.claude/hooks/session-start.sh > /dev/null 2>&1 && echo -e "${GREEN} Session start hook test passed${NC}" || echo -e "${YELLOW}  Session start hook test failed${NC}"
fi
echo ""
