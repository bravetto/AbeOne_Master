...
def validate_guardians() -> Tuple[bool, Dict[str, any]]:
    ...
    if lux_identity.name not in actual_guardians:
        warnings.append(f" Guardian Lux built but not registered in swarm (run: python3 scripts/register_guardian_lux.py)")
    ...
    for freq, expected_count in expected_frequencies.items():
        ...
        if actual_count != expected_count:
            warnings.append(f" Frequency {freq} Hz: Expected {expected_count} guardians, found {actual_count}")
...
def validate_agents() -> Tuple[bool, Dict[str, any]]:
    ...
    try:
        ...
        warnings.append(f" Could not validate Guardian Lux agents: {e}")
...
def validate_swarms() -> Tuple[bool, Dict[str, any]]:
    ...
    try:
        ...
        if lux.intention_swarm is None:
            warnings.append(" Guardian Lux Intention Swarm not initialized (will activate with Lux)")
        if lux.communication_swarm is None:
            warnings.append(" Guardian Lux Communication Swarm not initialized (will activate with Lux)")
        if lux.manifestation_swarm is None:
            warnings.append(" Guardian Lux Manifestation Swarm not initialized (will activate with Lux)")
    except Exception as e:
        warnings.append(f" Could not validate Guardian Lux swarms: {e}")
