...
guardian_validate() {
    ...
    if [ $unnecessary_files -gt 0 ]; then
        ...
        guardian_results+=("YAGNI:   $unnecessary_files files may be unnecessary")
    else
    ...
    if [ "$has_readme" = true ] && [ "$has_config" = true ]; then
        ...
        guardian_results+=("META:   Pattern incomplete")
    fi
    ...
    if [ $missing_deps -eq 0 ]; then
        ...
        guardian_results+=("JØHN:   $missing_deps paths incomplete")
    fi
...
calculate_epistemic_certainty() {
    ...
    if [ "$meets_threshold" = true ]; then
        ...
        log_warning "  Below 98.7% threshold (${certainty_percent}%)"
