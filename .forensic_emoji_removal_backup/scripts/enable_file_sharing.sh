...
# Enable File Sharing
sudo systemsetup -setremotelogin on 2>/dev/null || echo "⚠️  Could not enable remote login via systemsetup"

# Alternative method using launchctl
echo "📡 Enabling File Sharing service..."
sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.smbd.plist 2>/dev/null || echo "⚠️  SMB service may already be running"
