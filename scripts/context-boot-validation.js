...
class ContextBootValidator {
  ...
  async validateCurrentDirectory() {
    ...
    if (fs.existsSync(statusPath)) {
      ...
      const statusPatterns = [
        /\*\*Status\*\*[:\s*]+(?:|)?[:\s*]+\*\*(LEGACY|ACTIVE|ARCHIVE)\*\*/i,
        /Status[:\s*]+(?:|)?[:\s*]+(LEGACY|ACTIVE|ARCHIVE)/i,
      ...
      if (status === 'LEGACY' || status === 'ARCHIVE') {
        this.issues.push({
          type: 'CRITICAL',
          message: ` DRIFT DETECTED: Working in ${status} directory`,
    ...
    if (fs.existsSync(boundaryPath)) {
      try {
        ...
        if (boundary.status === 'LEGACY') {
          this.issues.push({
            type: 'CRITICAL',
            message: ` DRIFT DETECTED: Boundary file indicates LEGACY status`,
    ...
    if (fs.existsSync(masterIndexPath)) {
      ...
      if (legacyMatch) {
        this.issues.push({
          type: 'CRITICAL',
          message: ` DRIFT DETECTED: Master index indicates ${currentDirName} is LEGACY`,
  ...
  reportResults() {
    ...
    if (this.issues.length > 0) {
      ...
      console.log('\n  Warnings present - review above before proceeding.');
    } else {
  ...
  generateContextSummary() {
    ...
    if (fs.existsSync(statusPath)) {
      const statusContent = fs.readFileSync(statusPath, 'utf8');
      const statusMatch = statusContent.match(/\*\*Status\*\*[:\s*]+(?:|)?[:\s*]+\*\*(LEGACY|ACTIVE|ARCHIVE)\*\*/i);
      if (statusMatch) {
