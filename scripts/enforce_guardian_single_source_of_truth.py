...
def loud_warning(message: str):
    ...
    loud_print("\n" + " " * 20, Colors.WARNING_FG + Colors.BOLD)
...
def require_human_approval(action: str, details: str) -> bool:
    ...
    loud_print("  HUMAN APPROVAL REQUIRED ", Colors.YELLOW + Colors.BOLD + Colors.BLINK)
...
def enforce_single_source_of_truth(strict: bool = False, approve: bool = False, auto_heal: bool = False) -> int:
        ...
        for file_path_str in CRITICAL_FILES:
            ...
            try:
                ...
                if is_valid:
                    ...
                    if missing:
                        loud_error(f"     Missing: {', '.join(sorted(missing))}")
                    if extra:
                        loud_error(f"     Extra: {', '.join(sorted(extra))}")
    ...
    try:
        for file_path_str in OPTIONAL_FILES:
            ...
            try:
                ...
                if missing or extra:
                    loud_warning(f"     Differences found - Found {len(found)} guardians")
                ...
                loud_warning(f"     Error checking {file_path_str}: {e}")
                print()
    except Exception as e:
    ...
    if certainty_score >= 0.978:
        ...
        loud_warning(f"  System below threshold ({certainty_percent:.1f}% < 97.8%, gap: {gap*100:.1f}%)")
    ...
    try:
        if all_valid:
            ...
            if strict:
                ...
                loud_warning("  WARNING: Inconsistencies detected but not blocking")
                return 0
    except Exception as e:
