    ...
    async def _phase_1_validate_git_setup(self):
        ...
        for repo_name, repo_info in REPOSITORY_MAP.items():
            ...
            if not repo_path.exists():
                logger.warning(f"   ⚠️  Repository path not found: {repo_path}")
                continue
            ...
            try:
                ...
                if repo_info["remote"] in result.stdout:
                    ...
                    logger.warning(f"   ⚠️  {repo_name}: Remote mismatch")
            except Exception as e:
                logger.warning(f"   ⚠️  {repo_name}: Could not validate: {e}")
        ...
        for commit in self.report.commits:
            ...
            try:
                ...
                if result.returncode == 0 or "already exists" in result.stderr.lower():
                    ...
                    logger.warning(f"   ⚠️  {commit.repository}: Could not create branch: {result.stderr}")
                    commit.status = "failed"
                    commit.error = result.stderr
                    self.report.failed += 1
                    
            except Exception as e:
                logger.warning(f"   ⚠️  {commit.repository}: Error creating branch: {e}")
                commit.status = "failed"
                commit.error = str(e)
                self.report.failed += 1
    ...
    async def _phase_4_commit_with_reverence(self):
        ...
        for commit in self.report.commits:
            ...
            try:
                ...
                if not files_staged:
                    logger.info(f"   ⚠️  {commit.repository}: No files to commit (may already be committed)")
                    continue
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(f"   ⚠️  {commit.repository}: Commit failed: {result.stderr}")
                    commit.status = "failed"
                    commit.error = result.stderr
                    self.report.failed += 1
                    
            except Exception as e:
                logger.warning(f"   ⚠️  {commit.repository}: Error committing: {e}")
                commit.status = "failed"
                commit.error = str(e)
                self.report.failed += 1
        ...
        for commit in self.report.commits:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    logger.warning(f"   ⚠️  {commit.repository}: Push failed: {result.stderr}")
                    commit.status = "failed"
                    commit.error = result.stderr
                    self.report.failed += 1
                    
            except Exception as e:
                logger.warning(f"   ⚠️  {commit.repository}: Error pushing: {e}")
                commit.status = "failed"
                commit.error = str(e)
                self.report.failed += 1
