...
case $choice in
    1)
        ...
        if [ -d "AbeOne_Master" ]; then
            echo "  AbeOne_Master already exists. Removing..."
            rm -rf AbeOne_Master
        fi
...
if [ -f "package.json" ]; then
    ...
    if command -v npm &> /dev/null; then
        ...
        echo "  npm not found. Install Node.js to install dependencies."
    fi
fi
...
if [ -f "requirements.txt" ]; then
    ...
    if command -v pip3 &> /dev/null; then
        ...
        echo "  pip3 not found. Install Python to install dependencies."
    fi
fi
