...
run_test() {
    ...
    if eval "$test_command"; then
        ...
        if [ "$required" == "optional" ]; then
            echo -e "${YELLOW}  SKIPPED (Optional): $test_name${NC}"
            ((SKIPPED++))
            return 0
        else
...
if [ "$1" == "--skip-api-tests" ]; then
    ...
    if [ -z "$CLERK_TOKEN" ]; then
        echo -e "${YELLOW}  CLERK_TOKEN not set. API tests will show usage help.${NC}"
...
if [ -f "AiGuardian-Chrome-Ext-orbital/package.json" ]; then
    ...
    if [ -d "node_modules" ]; then
        ...
        echo -e "${YELLOW}  node_modules not found. Skipping npm tests.${NC}"
...
if command -v pytest &> /dev/null; then
    ...
    echo -e "${YELLOW}  pytest not found. Skipping pytest tests.${NC}"
    ((SKIPPED++))
fi
