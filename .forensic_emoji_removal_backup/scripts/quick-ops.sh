...
cmd_worktrees() {
    if [ -f "scripts/git-worktree-manager.sh" ]; then
        ...
        echo -e "${YELLOW}⚠️  Worktree manager not found${NC}"
    fi
}
...
cmd_hooks() {
    ...
    for hook in .claude/hooks/*.sh; do
        if [ -f "$hook" ] && [ -x "$hook" ]; then
            ...
            "$hook" "test" "0" > /dev/null 2>&1 && echo -e "${GREEN}✅ Passed${NC}" || echo -e "${YELLOW}⚠️  Warning${NC}"
        fi
    done
    echo ""
}
        ...
        for json_file in .claude/*.json; do
            if [ -f "$json_file" ]; then
                if python3 -m json.tool "$json_file" > /dev/null 2>&1; then
                    ...
                    echo -e "${YELLOW}⚠️  $(basename $json_file) - Invalid JSON${NC}"
                fi
            fi
        done
    fi
