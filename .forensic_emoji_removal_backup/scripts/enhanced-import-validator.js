...
class EnhancedImportValidator {
  ...
  loadProjects() {
    ...
    const activeMatch = content.match(/## 🎯 \*\*ACTIVE PROJECTS\*\*([\s\S]*?)## ⚠️/);
    if (activeMatch) {
    ...
    const legacyMatch = content.match(/## ⚠️ \*\*LEGACY PROJECTS\*\*([\s\S]*?)(?:## |$)/);
    if (legacyMatch) {
      ...
      for (const otherProject of this.projects) {
        if (otherProject.dir !== project.dir && 
          ...
          if (otherProject.status === 'LEGACY') {
            this.issues.push({
              type: 'CRITICAL',
              message: `⚠️ BLEED DETECTED: ${filePath}`,
            ...
            this.warnings.push({
              message: `⚠️ Cross-project import: ${filePath}`,
      ...
      if (packageJson.repository && packageJson.repository.url) {
        ...
        for (const otherProject of this.projects) {
          if (otherProject.dir !== project.dir && 
            ...
            this.warnings.push({
              message: `⚠️ Repository URL may reference wrong project: ${relativePath}`,
              details: `URL: ${repoUrl}`,
        ...
        if (typeof depVersion === 'string' && depVersion.startsWith('file:')) {
          ...
          for (const otherProject of this.projects) {
            if (otherProject.dir !== project.dir && 
                relativeToWorkspace.startsWith(otherProject.dir)) {
              this.warnings.push({
                message: `⚠️ Local dependency may cross project boundary: ${relativePath}`,
  ...
  reportResults() {
    ...
    if (this.issues.length > 0) {
      ...
      console.log('\n⚠️  Warnings present - review above.');
    } else {
