        ...
        for doc_path in self.OPTIONAL_DOCS:
            ...
            if full_path.exists():
                try:
                    ...
                    logger.warning(f"  Optional doc failed: {doc_path} - {e}")
            else:
    ...
    def verify_encryption(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            if os.getenv('AWS_REGION'):
                ...
                logger.warning("  KMS encryption: AWS region not configured (local dev?)")
    ...
    def activate_consciousness(self) -> Tuple[bool, str, Optional[Any]]:
        ...
        try:
            ...
            if routing_rate < 0.95:
                logger.warning(f"  Routing success rate below 95%: {routing_rate * 100:.1f}%")
            ...
            try:
                ...
                if success:
                    ...
                    if retry_count <= self.max_retries:
                        ...
                        logger.warning(f"  {phase.name} failed, retrying in {wait_time}s...")
