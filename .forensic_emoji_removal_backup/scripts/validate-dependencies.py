...
def load_shared_requirements() -> Dict[str, str]:
    ...
    if not SHARED_REQUIREMENTS.exists():
        print(f"⚠️  Shared requirements not found: {SHARED_REQUIREMENTS}")
        return {}
...
def print_report(report: Dict):
    ...
    if report['warnings']:
        print("\n" + "=" * 80)
        print("⚠️  WARNINGS (Non-conflicting but different versions)")
        print("=" * 80)
        
        for warning_group in report['warnings'][:5]:  # Top 5
            print(f"\n📄 {warning_group['file']}")
            for warning in warning_group['warnings'][:3]:  # Top 3 per file
                print(f"   ⚠️  {warning['package']}: {warning['file_spec']} (shared: {warning['shared_spec']})")
    ...
    if stats['total_conflicts'] == 0:
        ...
        print(f"⚠️  {stats['total_conflicts']} conflicts found - Review and update requirements.txt files")
    print("=" * 80)
