...
class CloudflarePagesAutoBind:
    ...
    def get_pages_project_id(self, account_id: str, project_name: str) -> Optional[str]:
        ...
        try:
            ...
            print(f"  Pages project not found: {project_name}")
            return None
    ...
    def verify_ssl_status(
        ...
        print(f"  SSL certificate not active after {max_wait}s (may still be provisioning)")
        return False
    ...
    def execute(
        ...
        if not self.bind_domain_to_pages(account_id, project_name, domain):
            print("  Domain binding failed (may already be bound)")
        ...
        if subdomain:
            ...
            if not self.bind_domain_to_pages(account_id, project_name, full_subdomain):
                print("  Subdomain binding failed (may already be bound)")
