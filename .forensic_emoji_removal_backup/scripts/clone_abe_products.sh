...
git clone $REPO_URL --branch $BRANCH --single-branch --depth 1 $TEMP_DIR || {
    echo "⚠️  Could not clone from GitHub. Trying Abeflows-orbital..."
...
if [ -d "$SOURCE_DIR/abedesigns" ]; then
    ...
    echo "⚠️  AbëDESIGNs not found in source"
fi
...
if [ -d "$SOURCE_DIR/abevisions" ]; then
    ...
    echo "⚠️  AbëViSiONs not found in source"
fi
...
if [ -d "$SOURCE_DIR/abevoices" ]; then
    ...
    echo "⚠️  AbëVOiCEs not found in source"
fi
...
if [ -d "$SOURCE_DIR/abegenius" ]; then
    ...
    echo "⚠️  AbëG.E.N.i.U.S not found in source"
fi
...
if [ -d "$SOURCE_DIR/abethinks" ]; then
    ...
    echo "⚠️  AbëTHINKs not found in source"
fi
...
if [ -d "$SOURCE_DIR/abeloves" ]; then
    ...
    echo "⚠️  AbëLOVEs not found in source"
fi
