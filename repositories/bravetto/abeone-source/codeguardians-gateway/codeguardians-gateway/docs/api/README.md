...
##  Guard Service Architecture

** Important**: Guard services run on internal Docker network ports only. All external access must go through the unified gateway endpoint: `POST /api/v1/guards/process`. Direct access to individual service endpoints is not available.
