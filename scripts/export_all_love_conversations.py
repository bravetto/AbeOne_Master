...
OUTPUT_DIR = Path(__file__).parent.parent / "products/abeloves/conversations"
...
def get_all_conversations():
    ...
    # Get Kristin conversations (PRIME: Extract from both text and attributedBody)
    ...
    for row in cursor:
        # PRIME: Parse text from either field (already operational)
        text = parse_message_text(row['text'], row['attributedBody'])
        
        if text:  # Only add if we successfully extracted text
    ...
    # Get Addis conversations (PRIME: Extract from both text and attributedBody)
    ...
    # Get group chats (PRIME: Extract from both text and attributedBody)
