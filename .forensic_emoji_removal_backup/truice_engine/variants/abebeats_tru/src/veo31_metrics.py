    ...
    def _generate_recommendations(self) -> List[str]:
        ...
        if self.metrics.get_success_rate() < 0.7:
            recommendations.append(
                f"⚠️ Overall success rate is {self.metrics.get_success_rate():.1%} - "
        ...
        if self.metrics.avg_quality_score < 0.6:
            recommendations.append(
                f"⚠️ Average quality score is {self.metrics.avg_quality_score:.2f} - "
        ...
        for key, metrics in self.pattern_metrics.items():
            if metrics.total_generations >= 5 and metrics.get_success_rate() < 0.5:
                recommendations.append(
                    f"⚠️ Pattern '{key}' has low success rate ({metrics.get_success_rate():.1%}) - "
