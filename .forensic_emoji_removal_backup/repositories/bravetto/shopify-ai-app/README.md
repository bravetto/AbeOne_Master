# Bravetto AI Chat - Shopify Integration

🤖 A powerful AI-powered chat assistant for Shopify stores that provides intelligent customer support using your store's data.

## 🔒 READ-ONLY ACCESS GUARANTEE
This app has **read-only permissions** to your Shopify store. It never modifies your products, pages, orders, or any store content. We only read your data to create an intelligent knowledge base for customer support.

**What we READ:** Products, pages, blog articles, collections  
**What we NEVER modify:** Your store content, customer data, orders, or settings

This is a Shopify app built with [Remix](https://remix.run) framework.
...
This app now uses a small shared-auth module (`app/lib/shared-auth.ts`) to:

- Validate App Proxy signatures and timestamp freshness
- Mint short-lived internal JWTs for server-to-server calls

Environment variables required:

- `SHOPIFY_API_SECRET` (for App Proxy HMAC)
- `INTERNAL_JWT_SECRET` (for internal JWT minting)
