...
class Settings(BaseSettings):
    ...
    def _load_abekeys_credentials(self) -> None:
        ...
        try:
            ...
            if abekeys_loader.reader:
                ...
                logger.info("AbëKEYS credentials loaded successfully")
        except Exception as e:
            logger.debug(f"AbëKEYS not available: {e}")

    def _load_aws_secrets(self) -> None:
        """Load secrets from AWS Secrets Manager if enabled (second priority)."""
        ...
        try:
            ...
            # Override environment variables with AWS secrets (only if not already set by AbëKEYS)
            for key, value in aws_secrets.items():
                if value and not os.environ.get(key):  # Only override if not already set
    ...
    def __init__(self, **kwargs):
        """Initialize settings with credential loading priority: AbëKEYS > AWS > Env Vars."""
        # Load AbëKEYS credentials first (highest priority)
        self._load_abekeys_credentials()
        # Load AWS secrets second (fallback)
        self._load_aws_secrets()
        # Initialize settings (will use environment variables as final fallback)
        super().__init__(**kwargs)

    model_config = SettingsConfigDict(
        # Note: env_file removed - using AbëKEYS vault instead
        # env_file=".env",  # REMOVED: Using AbëKEYS vault for credentials
