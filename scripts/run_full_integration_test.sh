...
if ! command -v stripe &> /dev/null; then
    ...
    if [[ "$OSTYPE" == "linux-gnu"* ]]; then
        ...
        echo -e "${YELLOW}  Please install Stripe CLI manually for webhook testing${NC}"
    fi
fi
...
if command -v stripe &> /dev/null; then
    ...
    echo -e "${YELLOW}  Stripe CLI not available - webhook testing will be skipped${NC}"
fi
...
echo ""
echo -e "${YELLOW}  Requires Configuration:${NC}"
