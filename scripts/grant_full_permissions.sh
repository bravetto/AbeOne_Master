...
if [ "$EUID" -ne 0 ]; then 
    echo "  Some permissions require administrator access"
...
if [ -f "$HOME/Library/Messages/chat.db" ]; then
    if [ -r "$HOME/Library/Messages/chat.db" ]; then
        ...
        echo "     Messages database exists but may not be readable"
        echo "   → Full Disk Access will fix this"
    fi
else
    echo "     Messages database not found (may need Full Disk Access)"
fi
echo ""
