#!/usr/bin/env node
/**
 *  Phase 1 File Creation Script (Node.js) 
 * 
...
// Create directories
const hooksDir = path.join(SCRIPT_DIR, 'hooks')
const utilsDir = path.join(SCRIPT_DIR, 'utils')
...
// Write files
const files = [
  { path: path.join(hooksDir, 'useBacklogQuery.ts'), content: useBacklogQuery },
  { path: path.join(hooksDir, 'useActivitiesQuery.ts'), content: useActivitiesQuery },
  { path: path.join(hooksDir, 'usePreferences.ts'), content: usePreferences },
  { path: path.join(utilsDir, 'indexedDB.ts'), content: indexedDB },
]

files.forEach(({ path: filePath, content }) => {
  fs.writeFileSync(filePath, content, 'utf8')
  console.log(\` Created \${path.relative(SCRIPT_DIR, filePath)}\`)
})

console.log('')
console.log(' Phase 1 files created successfully! ')
console.log('Pattern: CREATE × FILES × PHASE1 × ONE')
