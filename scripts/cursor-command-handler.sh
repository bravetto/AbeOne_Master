...
case "$COMMAND_NAME" in
    ...
    "disk-onboard")
        ...
        ;;
    "seal-one")
        exec python3 "$SCRIPT_DIR/abeone-seal-one.py" "$@"
        ;;
    "yagni")
        exec python3 "$SCRIPT_DIR/yagni_guardian.py" "$@"
        ;;
    "delta-check")
        exec python3 "$SCRIPT_DIR/delta-check-engine.py" "$@"
        ;;
    *)
        echo " Unknown command: $COMMAND_NAME"
        echo "Available commands: manifest, path-health, converge, ae, prime, create, aeon, val, revenue, push-suite, trigger, flow, axiom, epistemic, pattern, sync, memory, kernel, validate, disk-onboard, seal-one, yagni, delta-check"
        exit 1
        ;;
esac
