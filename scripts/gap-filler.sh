...
if [ -f ".gitignore" ]; then
    ...
    if ! grep -q "^\.claude/.*\.log$" .gitignore 2>/dev/null; then
        echo -e "${YELLOW}  Adding .claude log entries to .gitignore${NC}"
        {
            echo ""
    ...
    if ! grep -q "^\.claude/operational-status\.json$" .gitignore 2>/dev/null; then
        echo -e "${BLUE}ℹ  Note: operational-status.json can be tracked or ignored${NC}"
    fi
...
./scripts/health-check.sh > /dev/null 2>&1 && echo -e "${GREEN} Health check passed${NC}" || echo -e "${YELLOW}  Health check had warnings${NC}"
