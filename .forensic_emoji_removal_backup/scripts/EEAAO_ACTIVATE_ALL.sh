...
if [ -f "AIGuards-Backend-orbital/scripts/register_guardians_uptc.py" ]; then
    ...
    python3 scripts/register_guardians_uptc.py || echo "⚠️ UPTC registration may have failed, continuing..."
    cd "$PROJECT_ROOT"
else
    echo "⚠️ UPTC registration script not found, skipping..."
fi
echo ""
...
if [ -f "scripts/activate_guardian_swarm.sh" ]; then
    ...
    bash scripts/activate_guardian_swarm.sh || echo "⚠️ Guardian swarm activation may have failed, continuing..."
else
    echo "⚠️ Guardian swarm activation script not found, skipping..."
fi
echo ""
...
if [ -f "scripts/activate_all_149_agents.py" ]; then
    ...
    python3 scripts/activate_all_149_agents.py || echo "⚠️ Agent activation may have failed, continuing..."
else
    echo "⚠️ Agent activation script not found, skipping..."
fi
echo ""
...
try:
    ...
    print(f'⚠️ Full Monty initialization may have failed: {e}')
" || echo "⚠️ Full Monty initialization may have failed, continuing..."
else
    echo "⚠️ Full Monty orchestrator not found, skipping..."
fi
echo ""
...
try:
    ...
    print(f'⚠️ Operationalization may have failed: {e}')
" || echo "⚠️ Operationalization may have failed, continuing..."
else
    echo "⚠️ Operationalization script not found, skipping..."
fi
echo ""
...
if [ -f "scripts/register_guardian_lux.py" ]; then
    ...
    python3 scripts/register_guardian_lux.py || echo "⚠️ Guardian Lux activation may have failed, continuing..."
else
    echo "⚠️ Guardian Lux activation script not found, skipping..."
fi
echo ""
