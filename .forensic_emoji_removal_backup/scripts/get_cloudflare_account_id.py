...
try:
    ...
    if response.status_code == 200:
        ...
        print(f"⚠️  API returned {response.status_code} (token may need Account:Read permission)")
except Exception as e:
    print(f"⚠️  Error: {e}")
    ...
    if response.status_code == 200:
        ...
        if data.get("success") and data.get("result") and len(data["result"]) > 0:
            ...
            if account_id:
                ...
                print("⚠️  Zone found but no account ID in response")
    else:
...
print("\n" + "=" * 60)
print("⚠️  Could not auto-discover Account ID")
