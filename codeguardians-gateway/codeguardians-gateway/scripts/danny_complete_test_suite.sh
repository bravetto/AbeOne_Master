...
for pattern in "${SCENARIO_PATTERNS[@]}"; do
    ...
    case "$pattern" in
        ...
        flow_table_exhaustion|target_group_saturation|keep_alive_mismatch|dns_dead_ip|rst_pattern)
            python3 scripts/test_aws_nlb_failure_patterns.py \
                --url http://localhost:9001 \
                --json > "$RESULTS_DIR/nlb_${pattern}.json" 2>&1 || true
            ;;
        timeout_cascade)
            python3 scripts/test_aws_linkerd_integration_patterns.py \
                --url http://localhost:9001 \
...
run_and_capture "AWS NLB Test Script" "python3 scripts/test_aws_nlb_failure_patterns.py --list 2>&1 || python3 -c 'import scripts.test_aws_nlb_failure_patterns; print(\"OK\")'"
...
if curl -s http://localhost:8000/health > /dev/null 2>&1; then
    ...
    python3 scripts/test_aws_linkerd_deployment.py \
        --url http://localhost:8000 \
        --no-linkerd \
